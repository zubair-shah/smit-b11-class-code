...
const fetchPetDetails = async ({ queryKey }) => {
  ...
  try {
    const response = await fetch(`https://pets-v2.dev-apis.com/pets?id=${petId}`)
    if (!response.ok) {
    ...
    return {
      ...
      photos: [
        { large: "/placeholder.svg?height=500&width=600", medium: "/placeholder.svg?height=300&width=400" },
        { large: "/placeholder.svg?height=500&width=600", medium: "/placeholder.svg?height=300&width=400" },
        { large: "/placeholder.svg?height=500&width=600", medium: "/placeholder.svg?height=300&width=400" },
      ],
      contact: {
...
function Enhanced_Details() {
  ...
  const { data: pet, isLoading, error } = useQuery(["pet", id], fetchPetDetails)
  ...
  const handleFavorite = () => {
    ...
    enqueueSnackbar(favorite ? "Removed from favorites" : "Added to favorites", {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/cart/add", {
        ...
        body: JSON.stringify({
          ...
          petName: pet.name,
          petPrice: petPrice,
          petImage: pet.photos?.[0]?.large || pet.photos?.[0]?.medium || "/placeholder.svg?height=300&width=400",
          quantity: 1,
        }),
      })
      ...
      if (response.ok) {
        ...
        enqueueSnackbar(`🎉 ${pet.name} has been added to your cart! Complete the adoption process in checkout.`, {
          variant: "success",
  ...
  if (error || !pet) {
    return (
      <Container maxWidth="md" sx={{ py: 8, textAlign: "center" }
        ...
        < Button variant="contained" onClick={() => navigate("/")} sx={{ borderRadius: "50px" }}>
          Back to Home
        </Button>
      </Container>
    )
    ...
    <>
            ...
            < Grid item xs={12} md={6} >
              <PetImage
                image={
                  ...
                  images[selectedImageIndex]?.medium ||
                    ...
                    <Grid container spacing={1} >
                      {
                        images.slice(0, 4).map((photo, index) => (
                          <Grid item xs={3} key={index} >
                            <ThumbnailImage
                              src={photo.medium || photo.large || "/placeholder.svg?height=80&width=80"}
            ...
            <Grid item xs={12} md={6} >
              <InfoCard elevation={0}>
                ...
                < Box display="flex" gap={1} mb={3} flexWrap="wrap" >
                  <Chip icon={
                    <Pets />} label={pet.species} color="primary" variant="outlined" />
                  {pet.breeds?.primary && <Chip label={pet.breeds.primary} color="secondary" variant="outlined" />}
                  <Chip icon={pet.gender === "Male" ? <Male /> : <Female />} label={pet.gender} variant="outlined" />
                  <Chip icon={
                    <CalendarToday />} label={pet.age} variant="outlined" />
                </Box>
                ...
                <Box display="flex" gap={2} mb={3} >
                  ...
                  < ActionButton variant="outlined" startIcon={< Share />} onClick={handleShare} >
                    Share
                  </ActionButton>
                </Box>
                ...
                <Box display="flex" flexDirection="column" gap={2} >
                  <CartButton
                    ...
                    petName={pet.name}
                    petPrice={petPrice}
                    petImage={images[0]?.large || images[0]?.medium || "/placeholder.svg?height=300&width=400"}
                    onLoginRequired={handleLoginRequired}
                  />

                  <AdoptButton fullWidth size="large" onClick={handleAdopt} startIcon={< Pets />}>
                    Adopt {pet.name}
                  </AdoptButton>
                </Box>
              </InfoCard>
      ...
      >
        ...
        < DialogContent sx={{ textAlign: "center" }}>
          <Typography variant="body1" color="text.secondary" paragraph >
            You're about to start the adoption process for <strong>{pet.name}</strong>. This will add them to your cart
            where you can complete the adoption.
          </Typography>
        ...
        < DialogActions sx={{ p: 3, justifyContent: "center", gap: 2 }}>
          <Button onClick={() => setAdoptDialogOpen(false)} variant="outlined" sx={{ borderRadius: "50px", px: 3 }}>
            Cancel
          </Button>
          ...
          < AdoptButton
            ...
            disabled={adopting}
            startIcon={adopting ? <CircularProgress size={20} color="inherit" /> : <ShoppingCart />}
            sx={{ px: 4 }}
          >
